...
function useIsRtl() {
  ...
  React.useEffect(() => {
    // Check for RTL direction in document or html element
    ...
    const observer = new MutationObserver(mutations => {
      mutations.forEach(mutation => {
        if (mutation.attributeName === 'dir') {
          const newDir = document.documentElement.dir || document.body.dir || 'ltr';
          setIsRtl(newDir === 'rtl');
        }
      });
    });
...
function Sidebar({
  ...
  // Default side based on RTL setting if not explicitly specified
...
function SidebarTrigger({ className, onClick, ...props }: React.ComponentProps<typeof Button>) {
  ...
  return (
    <Button
      ...
      {...props}
    >
      <PanelLeftIcon className={isRtl ? 'rotate-180' : ''} />
...
function SidebarMenuSub({ className, ...props }: React.ComponentProps<'ul'>) {
  ...
  return (
    <ul
      ...
      className={cn(
        'flex min-w-0 translate-x-px flex-col gap-1 border-l py-0.5',
        isRtl ? 'border-r mr-3.5 pr-2.5 pl-0 border-l-0' : 'border-l ml-3.5 pl-2.5 pr-0',
        'group-data-[collapsible=icon]:hidden',
        className
      )}
      data-rtl={isRtl ? 'true' : 'false'}
